...
custom_barplot <- function(bar_data, lipid_class, abundance_level, y_label = ""){
  ...
  bar_data %>%
    ...
    geom_point(position = position_jitterdodge(dodge.width=dodge_width, jitter.width = 0.3), shape = 21, alpha = 0.5) +
...
```{r, high_bar_forest, out.width=c('50%', '50%'), fig.show='hold', fig.height = 6, eval = any_high, fig.path=fig_out}
...
cat("All median values for this class <= 1 so will be in the low abundance set")
```
...
```{r, low_bar_forest, out.width=c('50%', '50%'), fig.show='hold', fig.height = 6, eval = any_low, fig.path=fig_out}
...
cat("All median values for this class > 1 so will be in the high abundance set")
```
