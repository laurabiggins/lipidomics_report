...
```{r, results = "asis"}
output_text <- function(x, size = 12) {
  sprintf("<span style='font-size: %spx;'>%s</span>", size, x)
}

if(nrow(other_pie_data) > 0) {
  cat(output_text('Lipids in "other" category  \n', size = 24))
  cat("  ")
  cat(output_text('This is a breakdown of the categories that comprise the "other" category in the charts above.', size = 14))
}
```
