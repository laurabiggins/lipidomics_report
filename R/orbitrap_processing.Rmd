...
output_file: lip
params:
  #metadata_file: D:/projects/lipidomics/example_data/Feb_22/metadata.txt
  #input_file: D:/projects/lipidomics/example_data/Feb_22/Con_Dis.tsv
...
# change this to a warning and just remove any extra columns
assertthat::validate_that(
  all(colnames(all_data) %in% c(meta_info$sample_name, lipid_name_column)),
  msg = cat(paste(
    "Unexpected column name(s) found in all_data file: ", 
    paste0(colnames(all_data)[!(colnames(all_data) %in% c(meta_info$sample_name, lipid_name_column))],
           collapse = ", "),
...
if(!all(colnames(all_data) %in% c(meta_info$sample_name, lipid_name_column))){
  all_data <- all_data |> 
    dplyr::select(lipid_name_column, meta_info$sample_name)
}
